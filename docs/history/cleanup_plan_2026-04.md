# snake-dqn Cleanup Plan (HISTORICAL — superseded)

> **Status (2026-06-30): completed / obsolete.** This plan was largely executed
> during the 2026-06-28 single-model consolidation. Every file it targets for
> deletion — `sqlite_repository.py`, `gru_network.py`, `model_mixins.py`,
> `noisy_linear.py`, the entire `src/ui/` PyQt5 GUI — is already gone, and the
> abstraction cuts (Policy ABC collapse, DuelingMixin → `dueling_q` helper) are
> done. Kept only as a record of the reasoning. Do not action it against the
> current tree; see [ml_algorithm.md](../ml_algorithm.md) and the README for the
> present design.
