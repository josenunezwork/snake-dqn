...
def validate_serialized_optimizer_state(
    ...
    for parameter_id, entry in entries.items():
        ...
        if (
            parameter_id in expected_shapes
            and tuple(exp_avg.shape) != expected_shapes[parameter_id]
        ):
...
class ApexRecipe:
    ...
    def local(
        ...
        *,
        ...
        return cls._build(
            ...
            grad_clip_norm=10.0,
            priority_alpha=cfg.apex.priority_alpha,
            priority_eps=cfg.apex.priority_epsilon,
            beta_clock="successful_local_sample",
            world=asdict(cfg.game),
            replay_geometry={
                "batch_size": cfg.apex.batch_size if batch_size is None else batch_size,
                "capacity": cfg.apex.buffer_size if replay_capacity is None else replay_capacity,
                "warmup_samples": (
                    cfg.apex.min_buffer_size if min_replay_size is None else min_replay_size
                ),
                "beta_schedule": "increment_per_successful_local_sample",
                "beta_increment": cfg.training.priority_beta_increment,
            },
            runtime={
                "mode": "local_apex_policy",
                "training": True,
                "distributed": False,
    ...
    def distributed(
        ...
        *,
        ...
        beta_frames: int = 0,
        initial_beta_clock: int = 0,
    ) -> "ApexRecipe":
        """Build a coordinator-bound distributed descriptor, never ambient world values."""
        if (
            min(batch_size, replay_capacity, min_replay_size, beta_frames) <= 0
            or initial_beta_clock < 0
        ):
            raise ValueError(
                "distributed Apex recipe requires resolved replay geometry and beta horizon"
            )
        cfg = get_config()
        return cls._build(
            mode="distributed",
            ...
            target_clip=target_clip,
            grad_clip_norm=float(
                cfg.training.grad_clip_norm if grad_clip_norm is None else grad_clip_norm
            ),
            priority_alpha=priority_alpha,
            priority_eps=priority_eps,
            beta_clock="successful_distributed_sample",
            world=world,
            runtime=runtime,
            replay_geometry={
            ...
            exploration={
                "epsilon_base": cfg.apex.epsilon_base,
                "epsilon_alpha": cfg.apex.epsilon_alpha,
                "selection": "epsilon_greedy_actor_namespace",
                "actor_scaling": dict(actor_scaling),
            },
    ...
    def _build(
        ...
        *,
        ...
        seed_identity: Mapping[str, Any],
        replay_geometry: Mapping[str, Any],
